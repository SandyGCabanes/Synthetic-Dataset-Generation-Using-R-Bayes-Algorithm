...
# ~~~~~~~~~~~~~~~~~
library(dplyr)
df_sal <- read.csv("df_sal.csv")
syn_df_sal <- read.csv("syn_df_sal.csv")

df_sal <- as.data.frame(df_sal) %>%
  select(-X) %>%
  mutate(across(everything(), as.factor))
syn_df_sal <- as.data.frame(syn_df_sal) %>%
  select(-X) %>%
  mutate(across(everything(), as.factor))
...
str(freq_combined)   
summary(freq_combined)  

# 2. single column first ----

colvals <- names(syn_df_sal)
plot_value <- freq_combined %>%
...
present_vars <- unique(freq_combined$variable)
idx <- which(colvals %in% present_vars)

png_dir <- file.path(tempdir(), "plots_dir")
