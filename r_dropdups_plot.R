# setwd("C:/Users/sandy/Google_workspace/_google_sheets_survey/dep_annual_survey_2024/synthesized_data_using_r/github_bnlearn_full/n774_restart0/deduped")
...
syn_df_full <- read.csv("syn_df_full.csv", stringsAsFactors = TRUE)
dup_check <- read.csv("dup_check_full.csv", stringsAsFactors = TRUE)
all_factors <- df_full %>% select(where(is.factor))

# clean up X column if any 
df_full <- df_full %>% select(-any_of("X"))
syn_df_full <- syn_df_full %>% select(-any_of("X"))
dup_check <- dup_check %>% select(-any_of("X"))

# create combinedfactors columns both original and synthetic ----
df_full_comb <- df_full %>%
  mutate(combinedfactors = do.call(paste, c(all_factors, sep = "|")))
syn_df_full_comb <- syn_df_full %>%
  mutate(combinedfactors = do.call(paste, c(all_factors, sep = "|")))

# anti-join to clean original and synthetic dfs by excluding combinedfactors rows ----
df_full_deduped <- df_full_comb %>%
  anti_join(dup_check, by = "combinedfactors")
syn_df_full_deduped <- syn_df_full_comb %>%
  anti_join(dup_check, by = "combinedfactors")

# deduped: drop the combinedfactors and X column for plotting ----
df_full_deduped <- df_full_deduped %>%   
  select(-combinedfactors)
syn_df_full_deduped <- syn_df_full_deduped %>%   
  select(-combinedfactors)
...
freq_df_full  <- get_freqs(df_full_deduped)
...
output_file <- "all_plots_deduped.html"
